...
TRIMP LSTM
----------
Predicts heart rate from power + TRIMP-based ATL/CTL/TSB as inputs.
Input at each timestep: [power, atl_pre, ctl_pre, tsb_pre] (4 features).

Architecture and all hyperparameters are identical to tss_lstm.py
so results are directly comparable.

Reads  : TRIMP_Bounds_Filtered_Dataset.zip  +  trimp_atl_ctl_tsb.csv
Writes : trimp_lstm.pt                      +  trimp_history.csv
"""
...
# ---------------------------------------------------------------------------
# Config — identical to tss_lstm.py
# ---------------------------------------------------------------------------

DATASET_PATH     = r"C:\Users\Gebruiker\Desktop\The_Project\TRIMP_Bounds_Filtered_Dataset.zip"
ATL_CTL_TSB_PATH = r"C:\Users\Gebruiker\Desktop\The_Project\trimp_atl_ctl_tsb.csv"
MODEL_SAVE       = r"C:\Users\Gebruiker\Desktop\The_Project\VS_Part\7_model\trimp_lstm.pt"
HISTORY_SAVE     = r"C:\Users\Gebruiker\Desktop\The_Project\VS_Part\7_model\trimp_history.csv"
...
def get_fatigue(lookup, athlete_id, csv_basename, date_counter):
    try:
        date_str = csv_basename[:10].replace("_", "-")
...
def normalize(rides, stats):
    return [
        (
            (f - stats["feat_mean"]) / (stats["feat_std"] + 1e-8),
...
class RideDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(features), torch.from_numpy(hr)
...
# ---------------------------------------------------------------------------
# Model — identical to tss_lstm.py
# ---------------------------------------------------------------------------

class TRIMPFatigueLSTM(nn.Module):
...
def main():
    ...
    print(f"Loading fatigue features from {ATL_CTL_TSB_PATH.split(chr(92))[-1]}...")
    ...
    for i, name in enumerate(["power", "atl_pre", "ctl_pre", "tsb_pre"]):
    ...
    model     = TRIMPFatigueLSTM(INPUT_SIZE, HIDDEN_SIZE, DROPOUT).to(DEVICE)
