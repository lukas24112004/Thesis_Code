...
Baseline LSTM
-------------
Predicts heart rate from power output only.
No lagged features, no latent vectors, no engineered features.
Serves as the comparison point for the fatigue-augmented model.

Input  : power (1 feature, downsampled every 10 seconds)
Output : heart rate
Split  : 70% athletes training, 30% athletes testing (by athlete, not by ride)
"""
...
MODEL_SAVE    = r"C:\Users\Gebruiker\Desktop\The_Project\VS_Part\7_model\baseline_lstm.pt"
HISTORY_SAVE  = r"C:\Users\Gebruiker\Desktop\The_Project\VS_Part\7_model\baseline_history.csv"

SEED         = 42
TRAIN_RATIO  = 0.70
DOWNSAMPLE   = 10       # take every 10th row (~every 10 seconds)
MIN_STEPS    = 60       # minimum timesteps after downsampling (~10 minutes)

MAX_STEPS    = 300      # truncate rides longer than this (~50 min at 10s)
...
def load_all_rides(dataset_path):
    """Returns dict: athlete_id -> list of (power_array, hr_array)."""
    athlete_rides = {}
...
class RideDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.from_numpy(power).unsqueeze(-1),  # (T, 1)
...
class BaselineLSTM(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        return self.fc(out).squeeze(-1)  # (B, T_max)
