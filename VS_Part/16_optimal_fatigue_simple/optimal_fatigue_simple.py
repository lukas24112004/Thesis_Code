"""
Optimal Fatigue State — Simple Model (No Latent Vector)
--------------------------------------------------------
Same TSB sweep as extension_optimal_fatigue_performance.py but using
the simple model without latent vector.

For each test athlete: fix CTL at their median, sweep TSB from -40 to +20,
run the power ramp at each TSB value, find the power where predicted HR
crosses 90% of their observed max HR.

The TSB with the highest estimated max power = optimal fatigue state.

Comparison with definitive model result will show whether the latent
vector was responsible for the backwards optimal fatigue prediction.
...
SAVE_LINE        = "/content/drive/MyDrive/Research_Project/simple_optimal_fatigue_line.png"
SAVE_BAR         = "/content/drive/MyDrive/Research_Project/simple_optimal_fatigue_bar.png"
SAVE_CSV         = "/content/drive/MyDrive/Research_Project/simple_optimal_fatigue_results.csv"
...
def estimate_max_power_at_tsb(model, ctl, tsb, hr_threshold, stats):
    atl          = ctl - tsb
...
def main():
    ...
    for i, athlete_id in enumerate(test_ids, 1):
        ...
        for tsb in TSB_VALUES:
            max_p = estimate_max_power_at_tsb(model, median_ctl, tsb, threshold, stats)
        ...
        if not np.all(np.isnan(curve_arr)):
        ...
        print(f"    → Optimal TSB: {optimal_tsb}  (max power: {optimal_power:.0f} W)\n"
    ...
    ax.set_title("Simple Model — Effect of Fatigue on Estimated Max Power\n"
                 "No latent vector — CTL fixed at athlete median", fontsize=11)
    ...
    ax2.bar(range(len(valid_rows)), valid_rows["optimal_tsb"], color="#1f77b4", edgecolor="none")
    ax2.axhline(0, color="black", linewidth=0.8, linestyle=":")
    mean_opt = valid_rows["optimal_tsb"].mean()
    ax2.axhline(mean_opt, color="#d62728", linewidth=1.5, linestyle="--",
    ...
    ax2.set_title("Simple Model — Optimal Fatigue State per Test Athlete\n"
    ...
    # Comparison summary
    print(f"\n--- Comparison with definitive model ---")
    print(f"  Definitive model: mean optimal TSB = -25 (most athletes at -40)")
    print(f"  Simple model:     mean optimal TSB = {mean_opt:.1f}")
    print(f"  If similar → problem is in the data, not model complexity")
    print(f"  If different → latent vector was partially responsible")
